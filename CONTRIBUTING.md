...
Thanks for your interest in improving BugForge! Security tooling thrives on community contributions — new payloads, detection signatures, and report templates are all incredibly valuable.
...
```bash
git clone https://github.com/YOUR_USERNAME/bugforge.git
cd bugforge
pip install -e ".[dev]"
pytest -q
```
...
4. **Responsibility.** Never add functionality designed for out-of-scope or destructive testing. BugForge is for authorized, responsible testing only.
...
3. Ensure `pytest` passes and `python -m bugforge --help` works
