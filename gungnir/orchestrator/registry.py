"""
Tool registry — definitions for every tool Gungnir can orchestrate.
...
This is the single source of truth for what tools Gungnir wraps.
"""
...
class InstallMethod(str, Enum):
    ...
    PIP = "pip"
    DOCKER = "docker"
    BUILTIN = "builtin"   # part of Gungnir itself
...
register(ToolDefinition(
    ...
    run_builder=lambda t, o: [
        ...
        "--output-dir", o.get("output_dir", "/tmp/gungnir-sqlmap"),
        "--json",
    ] if t else [],
...
# ============ BUILTIN (Gungnir native) ============

register(ToolDefinition(
    name="gungnir-secrets",
    category=ToolCategory.SECRET,
    description="Gungnir native secret scanner (no external deps)",
    binary="python3",
    install_method=InstallMethod.BUILTIN,
    install_command="",
    github="",
    timeout=30,
    run_builder=lambda t, o: ["python3", "-m", "gungnir", "vulns", "secrets", "--file", t, "--json"],
    json_parser=lambda s: __import__("json").loads(s) if s.strip() else [],
...
register(ToolDefinition(
    name="gungnir-cors",
    category=ToolCategory.CORS,
    description="Gungnir native CORS checker (no external deps)",
    binary="python3",
    install_method=InstallMethod.BUILTIN,
    install_command="",
    github="",
    timeout=30,
    run_builder=lambda t, o: ["python3", "-m", "gungnir", "vulns", "cors", "--url", t],
    json_parser=lambda s: [],
